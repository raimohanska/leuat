...
connE.onValue(function (conn) {
  ...
  function summary(groupBy) {
    return function () {
      ...
      return Bacon.fromNodeCallback(leuat, "aggregate", spec)
        ...
        .map(function (list) {
          return list
            ...
            .map(function (a) {
              return { name: a._id, leukoja: a.leukoja, muutos: a.muutos };
            });
        });
    };
  }
  ...
  function multiStats(queryParam, groups, interval, count) {
    ...
    return Bacon.combineTemplate(statArr).map(function (statsByDate) {
      var groupedResults = {};
      statsByDate.forEach(function (dayStats) {
        ...
        groups.forEach(function (groupId) {
          groupedResults[groupId] = (groupedResults[groupId] || []).concat(
            groupedDayStats[groupId] || 0
          );
        });
      });
  ...
  io.on("connection", function (socket) {
    ...
    Bacon.fromEvent(socket, "leuat")
      .takeUntil(discoE)
      .doLog("request by")
      .flatMapLatest(function (groupBy) {
        return currentSummary[groupBy];
      })
  ...
  app.get("/leuat/all/:interval/:count", function (req, res) {
    serveStats(
      multiStats(null, [null], req.params.interval, req.params.count),
      res
    );
  });
